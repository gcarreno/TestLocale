...
procedure GetFormatSettings(ALang:String; out fmts: TFormatSettings);
...
var
  ...
  {$ENDIF}
begin
  LatestLocale:= setlocale(__LC_ALL, PAnsiChar(ALang));
  // Revert to the system's default
  if LatestLocale = '' then
    LatestLocale:= setlocale(__LC_ALL, PAnsiChar(LatestLocale));
  for i := 1 to 12 do
    begin
